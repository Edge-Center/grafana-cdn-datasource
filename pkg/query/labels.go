...
func NewTimeSeriesLabels(data *statistics.ResourceStatisticsTimeSeriesData, metric string, qm models.QueryModel) map[string]string {
	...
	labels["label"] = GetMetricLabel(metric, models.PluginStrings)

	return labels
}

func NewTableLabels(data *statistics.ResourceStatisticsTableData, metric string, qm models.QueryModel) map[string]string {
	labels := make(map[string]string)
